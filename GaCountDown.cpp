...
bool CGaCountDown::ComputeColorCount( Byte ucDirIndex , Byte& ucColor , Byte& ucCountTime )
{
	...
	if(pConfig->sSpecFun[FUN_COUNT_DOWN].ucValue == COUNTDOWN_GAT5082004)
	{
	//左 直  右  不算人行
	for ( ucLaneIndex=0; ucLaneIndex<GA_MAX_LANE; ucLaneIndex++ )
	{
...
{ 					  
	...
	if(ucDirIndex == 0x3)
		m_newsGaSendBuf4D[GANEW_CHKDATA_INDEX] = 0x4^m_newsGaSendBuf4D[3]^m_newsGaSendBuf4D[4]^m_newsGaSendBuf4D[5]^m_newsGaSendBuf4D[6]^m_newsGaSendBuf4D[7]^m_newsGaSendBuf4D[8]^m_newsGaSendBuf4D[9]^m_newsGaSendBuf4D[10];
	return true;
}
