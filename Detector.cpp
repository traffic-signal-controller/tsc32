...
bool CDetector::IsHaveCarPhase(Byte uiPhase)
{
	...
	ucPhaseIndex = uiPhase -0x1;  //放行相位
	iDetCnt      = pManaKernel->m_sPhaseDet[ucPhaseIndex].iRoadwayCnt;//判断该相位有几个检测器
	iDetIndex = 0;
	while ( iDetIndex < iDetCnt )//该相位所有的检测器循环一遍
	{
		iDetId =pManaKernel->m_sPhaseDet[ucPhaseIndex].iDetectorId[iDetIndex] - 1; //检测器数组下标
