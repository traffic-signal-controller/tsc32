...
void CWirelessBtn::BackToAuto()
{
	...
	else if(m_ucLastManualSts==BUTTON_MANUAL_NEXT_STEP || m_ucLastManualSts==BUTTON_MANUAL_MANUAL) //当从手动下一步退出 则黄灯过渡3秒
	{
...
void CWirelessBtn::RecvMacCan(SCanFrame sRecvCanTmp)
{
	...
	if(RecvType == 0x2 && bTransitSetp == false) //RecvType == 0x2 表示接收按键信息,手控非过渡步
	{
		sWirelessBtnCanMsg = sRecvCanTmp ;
		bHandleWirelessBtbMsg = true ;    //无线手控线程可以处理按键信息
	}
	else
		return ;
}
...
{		
		...
		if(iAuto_Manul == 0x0) //自动状态
		{
			...
			else
			{
				...
				BackToAuto();
				CloseWirelessBtn();								
				m_pWirelessBtnTimer->Resettimeout();  //退出定时器超时计数置为0
			}
		}
...
Output:         无
Return:         bool值,true-需线程处理无线手控按键 false-无需处理
Date:            20141117
***************************************************************/
